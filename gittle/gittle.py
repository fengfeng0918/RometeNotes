...
class Gittle(object):
    ...
    def _format_ref(self, base, extra):
        if isinstance(extra,bytes):
            extra = str(extra, encoding = "utf-8") 
    ...
    def _wants_branch(self, branch_name=None):
        ...
        def wants_func(old):
            refs_key = self._format_ref_branch(branch_name)
            if not isinstance(refs_key,bytes):
                refs_key = bytes(refs_key,encoding="UTF-8")
            return {
                refs_key: sha
            }
        return wants_func
...
#             print(type(modified_files))
        ...
        # print("message：{0},\n author:{1},\n committer:{2}".format(message,author_msg,committer_msg))
        # print("clss:message：{0},\n author:{1},\n committer:{2}".format(type(message),type(author_msg),type(committer_msg)))
        
        message = bytes(message,encoding='UTF-8')
        committer_msg = bytes(committer_msg,encoding='UTF-8')
        author_msg = bytes(author_msg,encoding='UTF-8')
    ...
    def _refs_by_pattern(self, pattern):
        ...
        def item_filter(key_value):
            ...
            key, value = key_value
            return key.startswith(bytes(pattern,encoding="utf-8"))
    ...
    def import_refs(self, base, other):
        base = bytes(base,encoding='utf-8')
    ...
    def active_branch(self):
        ...
        if not x.startswith(SYMREF):
            ...
            symref = x[len(SYMREF):]
            # print(symref,type(symref))
            # print(self.REFS_BRANCHES,type(self.REFS_BRANCHES))
            if not symref.startswith(bytes(self.REFS_BRANCHES,encoding="UTF-8")):
                return None
            else:
